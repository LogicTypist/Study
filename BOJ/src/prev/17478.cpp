...
void print_underbars(int num){
  for(int i = 0; i < num; ++i){
    std::cout << "____";
  }
}

void recursive_professor(int depth, int current){
  print_underbars(current);
  ...
  if(depth == current){
    ...
    recursive_professor(depth, current + 1);
  }
  print_underbars(current);
